...
watch( 
//  props,  Funcionaría igual pero props.id es más especifico
  () => props.id, 
  () => {
    project.value = projectStore.projectList.find((project) => project.id === props.id);
    if (!project.value) {
      router.replace('/'); // .replace impide el regreso porque borra el historial .push permitiría el retroceso en la navegación
    }
  },
  {
    immediate: true, // Para que se muestre información desde el mismo momento en que se monte el componente
  },
);
</script>
